...
/* ================= MinesPanel ================= */
function MinesPanel({
// --- Replaced calcPayout: compressed Stake-like odds, first picks negative ---
function calcPayout(safeClicks, baseBet, minesCount) {
  const totalTiles = rows * cols;
  const safeTiles = totalTiles - minesCount;
  if (safeClicks <= 0) return { multiplier: 0, payout: 0, profit: -baseBet };
  // fair multiplier from combinatorics
  let numerator = 1, denominator = 1;
  for (let i = 0; i < safeClicks; i++) {
    numerator *= (safeTiles - i);
    denominator *= (totalTiles - i);
  }
  let fairMultiplier = (numerator / denominator) * (totalTiles / safeTiles);
  // apply base house edge
  fairMultiplier *= 0.99;

  // Force early picks to be losses (scale depends on mine count)
  const baseline = 0.6;
  let newMult;
  if (safeClicks === 1) {
    newMult = baseline + (minesCount / (rows*cols)) * 0.1; // ~0.6 - 0.7
  } else if (safeClicks === 2) {
    newMult = Math.max(0.75, baseline + (fairMultiplier - baseline) * 0.35);
  } else if (safeClicks === 3) {
    newMult = Math.max(0.9, baseline + (fairMultiplier - baseline) * 0.45);
  } else {
    // compress range toward baseline to lower rates, then boost later on low-mine games
    newMult = baseline + (fairMultiplier - baseline) * 0.45;
    if (safeClicks > 3 && minesCount <= 5) {
      newMult *= 1 + (safeClicks - 3) * 0.12;
    }
  }
  // Do not allow multipliers to be absurdly high
  if (!isFinite(newMult) || newMult <= 0) newMult = baseline;
  const payout = parseFloat((baseBet * newMult).toFixed(2));
  const profit = parseFloat((payout - baseBet).toFixed(2));
  return { multiplier: newMult, payout, profit };
}
