...
function computeMultiplier(safeRevealed){
  ...
  // Immediate cashout baseline (house takes small fee)
  if (safeRevealed === 0) return Math.max(0, 0.94 * 0.985)

  // Penalty stretch: fewer mines -> longer penalty
  const penaltyStretch = Math.max(3, Math.floor((25 - mines) / 3))

  // Base start: around 0.6, slightly higher if there are many mines
  const baseStart = 0.6 + (mines / totalTiles) * 0.05
  const forcedRange = 0.35 // amount to ramp during penalty stretch

  if (safeRevealed <= penaltyStretch) {
    const forced = baseStart + (safeRevealed / penaltyStretch) * forcedRange
    return Math.max(0, forced * 0.985)
  }

  // After penalty stretch, use combinatorial fair-ish multiplier then boost for low-mine games
  let numerator = 1, denominator = 1
  for (let i = 0; i < safeRevealed; i++) {
    numerator *= (safeTiles - i)
    denominator *= (totalTiles - i)
  }
  let fairMultiplier = (numerator / denominator) * (totalTiles / Math.max(1, safeTiles))
  fairMultiplier *= 0.985 // house edge

  // Boost scaling for later picks on low-mine games
  const safeRatio = safeTiles / totalTiles
  let mult = fairMultiplier
  if (safeRevealed > penaltyStretch) {
    const extra = safeRevealed - penaltyStretch
    const boostFactor = 1 + extra * (0.06 + safeRatio * 0.12)
    mult = fairMultiplier * boostFactor
  }

  // clamp and return
  if (!isFinite(mult) || mult <= 0) mult = baseStart * 0.985
